...
impl<Manager: 'static> NetConnection<Manager> {
    ...
    pub fn get_connection_info(&self) -> Option<NetConnectionInfo> {
        unsafe {
            let mut info: sys::SteamNetConnectionInfo_t = std::mem::zeroed();

            if sys::SteamAPI_ISteamNetworkingSockets_GetConnectionInfo(
                self.sockets,
                self.handle,
                &mut info,
            ) {
                Some(NetConnectionInfo::new(info))
            } else {
                None
            }
        }
