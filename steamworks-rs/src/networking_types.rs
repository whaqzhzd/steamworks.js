...
use crate::networking_sockets::{InnerSocket, NetConnection, NetPollGroup};
...
#[derive(Debug, Copy, Clone, Eq, PartialEq)]
pub enum NetConnectionEndReason {
    NetConnectionEnd(NetConnectionEnd),
    Customed(i32),
}

impl From<NetConnectionEndReason> for i32 {
    fn from(value: NetConnectionEndReason) -> Self {
        match value {
            NetConnectionEndReason::NetConnectionEnd(x) => x.into(),
            NetConnectionEndReason::Customed(x) => x,
        }
    }
}
...
pub struct InvalidEnumValue(i32);
...
impl NetConnectionInfo {
    pub fn new(inner: sys::SteamNetConnectionInfo_t) -> NetConnectionInfo {
        Self { inner }
    }
    ...
    pub fn end_reason(&self) -> Option<NetConnectionEndReason> {
        if self.inner.m_eEndReason
            == sys::ESteamNetConnectionEnd::k_ESteamNetConnectionEnd_Invalid as _
        {
            None
        } else {
            Some(self.inner.m_eEndReason.try_into().map_or_else(
                |e: InvalidEnumValue| NetConnectionEndReason::Customed(e.0),
                |o| NetConnectionEndReason::NetConnectionEnd(o),
            ))
        }
    }

    pub fn ip_v4(&self) -> Option<Ipv4Addr> {
        let ip = SteamIpAddr::from(self.inner.m_addrRemote);
        ip.get_ipv4()
    }

    pub fn port(&self) -> u16 {
        self.inner.m_addrRemote.m_port
    }
}
...
impl<Manager: 'static> ConnectionRequest<Manager> {
    ...
    pub fn accept(&mut self) -> SResult<()> {
        self.connection.accept()
    }

    pub fn reject(self, end_reason: NetConnectionEndReason, debug_string: Option<&str>) -> bool {
    ...
    pub fn set_connection_poll_group(&self, poll_group: &NetPollGroup<Manager>) {
        self.connection.set_poll_group(poll_group);
    }
}
...
pub struct DisconnectedEvent {
    remote: NetworkingIdentity,
    user_data: i64,
    end_reason: NetConnectionEndReason,
}
...
impl DisconnectedEvent {
    ...
    pub fn end_reason(&self) -> NetConnectionEndReason {
        self.end_reason
    }
}
...
impl<Manager> NetworkingMessage<Manager> {
    ...
    pub fn set_handle(&mut self, handle: u32) {
        unsafe { (*self.message).m_conn = handle }
    }
